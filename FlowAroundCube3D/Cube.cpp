...
// A point is inside the cube if it is inside the AABB
bool Cube::contains(const Vector3< real_t >& point) const
{
   return cubeAABB_.contains(point);
}

// An AABB is fully inside the cube if ALL 8 corners are inside
bool Cube::contains(const AABB& aabb) const
{
   Vector3< real_t > p[8];
...
// Uses parametric ray-AABB intersection: find t in [0,1] where ray crosses cube face
real_t Cube::delta(const Vector3< real_t >& fluid, const Vector3< real_t >& boundary) const
{
   WALBERLA_ASSERT(!contains(fluid))
   WALBERLA_ASSERT(contains(boundary))

   // Ray: P(t) = fluid + t * (boundary - fluid), find smallest t in (0,1] where ray enters cube
   ...
   for (uint_t i = 0; i < 3; ++i)
   {
      ...
      real_t t0 = (cubeAABB_.min(i) - fluid[i]) * invD;
      real_t t1 = (cubeAABB_.max(i) - fluid[i]) * invD;
   ...
   // tMin is the entry point of the ray into the cube
...
void Cube::setupBoundary(const std::shared_ptr< StructuredBlockForest >& sbfs, const BlockDataID flagFieldID)
{
   ...
   setupCubeBoundary(sbfs, flagFieldID);
}

void Cube::setBoundaryFromCellInterval(CellInterval& cells, const FlagField_T::flag_t flag, FlagField_T* flagField)
{
...
void Cube::checkConsistency(const std::shared_ptr< StructuredBlockForest >& sbfs, const BlockDataID flagFieldID)
{
   ...
   for (auto bIt = sbfs->begin(); bIt != sbfs->end(); ++bIt)
   {
      ...
      if (sbfs->getLevel(b) < depth){
         for( auto it = flagField->beginWithGhostLayer(1); it != flagField->end(); ++it ){
            ...
            WALBERLA_CHECK(!contains(cellCenter), "The cube must be completely located on the finest level")
         }
      }
   }
}

void Cube::setupCubeBoundary(const std::shared_ptr< StructuredBlockForest >& sbfs, const BlockDataID flagFieldID)
{
   const uint_t depth = sbfs->getDepth();
   for (auto bIt = sbfs->begin(); bIt != sbfs->end(); ++bIt)
   {
...
real_t wallDistanceCube::operator()(const Cell& fluidCell, const Cell& boundaryCell,
                                    const shared_ptr< StructuredBlockForest >& SbF, IBlock& block) const
{
   ...
   WALBERLA_CHECK(!cube_.contains(fluid),
      "fluid cell is contained in cube (" << fluid[0] << ", " << fluid[1] << ", " << fluid[2] << ")")
   WALBERLA_CHECK(cube_.contains(boundary),
      "boundary cell is not contained in cube (" << boundary[0] << ", " << boundary[1] << ", " << boundary[2] << ")")

   return cube_.delta(fluid, boundary);
}
