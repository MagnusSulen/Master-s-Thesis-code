...
#include "FlowAroundCubeStaticDefines.h"

using namespace walberla;

using BoundaryCollection_T = lbm::FlowAroundCubeBoundaryCollection< FlagField_T >;
using SweepCollection_T = lbm::FlowAroundCubeSweepCollection;
...
int main(int argc, char** argv)
{
   ...
   WALBERLA_LOG_INFO_ON_ROOT("Side length of the cube is resolved with " << setup.resolutionSphere << " lattice cells.")
   Cube Cube( setup );
   ...
   if (writeSetupForestAndReturn && mpi::MPIManager::instance()->numProcesses() == 1)
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT( "Benchmark run data:"
                                ...
                                "\n   + # cells per cube side:    " << setup.resolutionSphere <<
                                "\n   + total memory:        " << totalMemory / 1e9 << " [GB]" <<
                                "\n- simulation properties:  "
                                "\n   + cube pos.(x):          " << setup.sphereXPosition * setup.dxC << " [m]" <<
                                "\n   + cube pos.(y):          " << setup.sphereYPosition * setup.dxC << " [m]" <<
                                "\n   + cube pos.(z):          " << setup.sphereZPosition * setup.dxC << " [m]" <<
                                "\n   + cube half-side:       " << setup.sphereRadius * setup.dxC << " [m]" <<
   ...
   Cube.setupBoundary(blocks, ids.flagField);
   ...
   std::function< VelocityField_T::value_type (const Cell&, const Cell&, const shared_ptr< StructuredBlockForest >&, IBlock&) >
      wallDistanceFunctor = wallDistanceCube(Cube);
   ...
   {
      const std::string vtkFolder = "VTKCube_Re" + std::to_string(uint64_c(setup.reynoldsNumber));
      auto vtkOutput =
