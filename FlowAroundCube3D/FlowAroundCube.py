...
# ===== Turbulence model toggle =====
# Set to True to enable the Smagorinsky subgrid-scale model on top of
# the K17 collision operator. K17 already provides implicit LES
# behaviour, so additional Smagorinsky dissipation is recommended only
# when the simulation becomes unstable at high Reynolds numbers.
...
with CodeGeneration() as ctx:
    ...
    lbm_config = LBMConfig(
        ...
        compressible=True,
        subgrid_scale_model=(SubgridScaleModel.SMAGORINSKY, SMAGORINSKY_CS) if USE_SMAGORINSKY else None,
    ...
    generate_lbm_package(ctx, name="FlowAroundCube", collision_rule=collision_rule,
    ...
    generate_info_header(ctx, 'FlowAroundCubeInfoHeader',
    ...
    ctx.write_file("FlowAroundCubeStaticDefines.h", info_header.format(**infoHeaderParams))
