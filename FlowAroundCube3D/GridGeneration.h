...
void createSetupBlockForest(SetupBlockForest& setupBfs, const Setup& setup, const bool useMPIManager=false)
{
   ...
   Cube Cube( setup );
   CubeRefinementSelection CubeRefinementSelection( Cube, setup.refinementLevels );
   CubeBlockExclusion CubeBlockExclusion( Cube );

   setupBfs.addRefinementSelectionFunction(std::function<void(SetupBlockForest &)>(CubeRefinementSelection));
   setupBfs.addBlockExclusionFunction(CubeBlockExclusion);
