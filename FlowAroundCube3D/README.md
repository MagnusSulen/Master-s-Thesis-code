...
The showcase serves as a three-dimensional validation case for the
lattice Boltzmann framework. A cube is used because it shares the key
aerodynamic feature of many engineering geometries, namely sharp
edges with fixed separation points, while remaining simple enough to
compare against reference data from the literature.

The same framework can be reused for any bottom-fixed obstacle by
replacing the cube with a voxelized representation of an arbitrary
geometry. See the section "Adapting the code to a custom bottom-fixed
geometry" below.
...
- Subgrid model: toggleable Smagorinsky (off by default through the
  flag in `FlowAroundCube.py`); K17 alone already provides implicit
  LES behaviour
...
The shipped `.prm` defines:

| Parameter         | Value                                  |
|-------------------|----------------------------------------|
| Reynolds number   | 1000                                   |
| Mach (lattice)    | ~ 0.087  (`latticeVelocity = 0.05`)    |
| Cube side length  | `L = 1.0`                              |
| Domain            | `48L x 32L x 32L`                      |
| Coarse mesh size  | `dx_C = 0.25`  (4 cells per `L` on L0) |
| Refinement levels | 2  (L0, L1, L2)                        |
| Finest resolution | `dx_F = 0.0625`  (16 cells per `L`)    |
| Cells per block   | `16 x 16 x 16`                         |
| Cube position     | centred laterally, 16L from inlet      |

The refinement zone in `Cube.h` covers a region from `(12L, 12L, 12L)`
to `(22L, 20L, 20L)`, enclosing the cube and its near wake.

The default boundary conditions are:
...
collision operator. This is appropriate for the validation runs at
moderate Reynolds numbers (Re up to roughly 10^6 for a single cube).

Set `USE_SMAGORINSKY = True` when additional dissipation is needed for
stability at higher Reynolds numbers or on complex voxelized
geometries where sharp edges generate strong local strain rates.

The generated CUDA kernel is different for the two settings. After
changing the flag the application must be rebuilt with
`ninja FlowAroundCube` so that the code generation pipeline runs
again.
...
- Python 3 with `lbmpy`, `pystencils`, and `pystencils_walberla`
  (waLBerla pulls these in as build dependencies when code generation
  is enabled)
...
- `dragSphereRe_<Re>_meshLevels_<N>.csv` (legacy filename retained
  from the upstream showcase) contains the time series of drag and
  lift forces on the cube.
- `VTKCube_Re<Re>/` contains VTK output of the velocity and density
  fields at the frequency configured in `VTKWriter`.

## Adapting the code to a custom bottom-fixed geometry
...
custom bottom-fixed structure (a building, a vehicle on a flat plane,
a small offshore mast, a piece of equipment) the following changes are
required.

### 1. Voxelize the geometry

Use the `voxelizer/voxelize_stl.py` script in the parent repository to
convert a triangulated STL surface mesh into a binary voxel grid:
...
Write a class with the same public interface as `Cube` that:
...
expanded to cover the full voxel grid extent, and the wake refinement
zone should be set to roughly five characteristic lengths downstream
of the obstacle.

### 3. Cut the domain below the obstacle

For bottom-fixed structures (sea surface, ground, foundation slab),
the simulation domain should not extend below the base of the
obstacle. Two options exist.

**Option A — Domain trimming.** Set `domainSize` in the `.prm` so
that the vertical extent starts at the obstacle base. The full
domain is then air, and no cells are wasted on representing the
ground. This is appropriate when the obstacle has a flat base, or
when the foundation below the obstacle is uniform and does not need
to be resolved.

**Option B — Solid filling below the obstacle.** Voxelize so that
all cells below the obstacle base are flagged as solid. The
`BlockExclusion` functor in `GridGeneration.h` then excludes those
blocks at the block-forest level, leaving a step at the lower
domain boundary. This costs one block of solid cells along the
bottom but is the simplest way to represent an irregular foundation
that interacts with the flow (legs, columns, pillars).

For a flat foundation, Option A is preferred because it saves
memory. For an irregular foundation, Option B is required.
...
the obstacle BC entry in `.prm` from `Obstacle` (the
QuadraticBounceBack flag) to the corresponding NoSlip flag, and
adjust the boundary generator in `FlowAroundCube.py` accordingly.

A consequence of switching to `NoSlip` is that
`calculate_force_on_boundary` is not available, so force time series
...
`FixedDensity` clamps the pressure at the outlet to a fixed value,
which generates spurious reflections when the wake fluctuates
strongly. An extrapolation-type outflow lets the flow leave the
domain without imposing a pressure, which gives cleaner wake
statistics for complex obstacles. The corresponding boundary
generator is available in the `ExtrapolationOutflow` class from
`lbmpy.boundaries`.
...
The refinement-selection functor in `Cube.h` defines an AABB that
surrounds the cube and its near wake. For a complex obstacle, replace
the cube half-side with a representative dimension (typically the
obstacle height or the largest horizontal extent), and re-centre the
AABB on the obstacle bounding box.
...
- Adjusting the parameter file and refinement zone for a `48L x 32L
  x 32L` domain.
