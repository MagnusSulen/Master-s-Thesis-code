   ...
   WALBERLA_ROOT_SECTION()
   {
      ...
      if (logToStream_ and !dragResults.empty()){
         WALBERLA_LOG_RESULT_ON_ROOT(
            "force acting on square (in dimensionless lattice units of the coarsest grid - evaluated in time step "
...
void Evaluation::forceCalculation(const uint_t level)
{
   ...
   WALBERLA_LOG_INFO_ON_ROOT("forceCalculation called: level=" << level << " maxLevel=" << maxLevel_);
   if(level == maxLevel_){
      for (auto b : finestBlocks_){
         force_ += Vector3<double>(boundaryCollection_.ObstacleObject->getForce(b));
      }
      WALBERLA_LOG_INFO_ON_ROOT("force accumulated: " << force_);

      mpi::reduceInplace(force_, mpi::SUM);
      WALBERLA_ROOT_SECTION(){
         const double meanU2 = double_c(meanVelocity) * double_c(meanVelocity);

         const double cDRealArea = (double_c(8.0) * force_[0]) / (meanU2 * double_c(surfaceAreaSquare));
         const double cLRealArea = (double_c(8.0) * force_[1]) / (meanU2 * double_c(surfaceAreaSquare));
               ...
               if (flagField->isFlagSet(x, y, z, fluid)){
                  for (auto it = Stencil_T::beginNoCenter(); it != Stencil_T::end(); ++it){
                     ...
                     if (flagField->isFlagSet(nx, ny, nz, obstacle)){
                        WALBERLA_CHECK(blockLevel == finestLevel, "The square must be completely located on the finest level")
