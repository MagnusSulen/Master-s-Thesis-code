...
#include "Types.h"
#include "Setup.h"
#include "SquareCylinder2DFinalInfoHeader.h"
...
class Evaluation
{
 public:
        ...
        logToStream_(logToStream), logToFile_(logToFile)
   {
      ...
      diameterSquare      = double_c(2.0) * (double_c(setup_.squareHalfSide) * factor);
      surfaceAreaSquare   = math::pi * diameterSquare * diameterSquare;
      meanVelocity        = setup_.inflowVelocity; // (double_c(4.0) * setup_.inflowVelocity) / double_c(9.0);

      dragFilename_ = std::string("dragSquareRe_") + std::to_string(uint_t(setup_.reynoldsNumber)) + std::string("_meshLevels_") +  std::to_string(uint_t(setup_.refinementLevels + 1)) + std::string(".csv");
      ...
      WALBERLA_LOG_INFO_ON_ROOT(
         "Evaluation initialised:"
            "\n   + Square real area:                      " << surfaceAreaSquare
         << "\n   + Square side length:                  " << diameterSquare
         << "\n   + Square discrete area drag coefficient: " << AD_
         << "\n   + Square discrete area lift coefficient: " << AL_
      )
 ...
 protected:
   ...
   double diameterSquare;
   double surfaceAreaSquare;
   double meanVelocity;
