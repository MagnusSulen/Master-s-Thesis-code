...
void createSetupBlockForest(SetupBlockForest& setupBfs, const Setup& setup, const bool useMPIManager=false)
{
   ...
   Square square( setup );
   SquareRefinementSelection squareRefinementSelection( square, setup.refinementLevels );
   SquareBlockExclusion squareBlockExclusion( square );

   setupBfs.addRefinementSelectionFunction(std::function<void(SetupBlockForest &)>(squareRefinementSelection));
   setupBfs.addBlockExclusionFunction(squareBlockExclusion);
