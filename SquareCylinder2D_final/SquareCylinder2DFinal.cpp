...
//
//! \file SquareCylinder2DFinal.cpp
...
#include "SquareCylinder2DFinalStaticDefines.h"

using namespace walberla;

using BoundaryCollection_T = lbm::SquareCylinder2DFinalBoundaryCollection< FlagField_T >;
using SweepCollection_T = lbm::SquareCylinder2DFinalSweepCollection;
...
int main(int argc, char** argv)
{
   ...
   WALBERLA_LOG_INFO_ON_ROOT("Side length of the Square is resolved with " << setup.resolutionSquare << " lattice cells.")
   Square square(setup);
   ...
   if (writeSetupForestAndReturn && mpi::MPIManager::instance()->numProcesses() == 1)
   {
      ...
      WALBERLA_LOG_INFO_ON_ROOT( "Benchmark run data:"
                                ...
                                "\n   + # cells square side:    " << setup.resolutionSquare <<
                                "\n   + total memory:        " << totalMemory / 1e9 << " [GB]" <<
                                "\n- simulation properties:  "
                                "\n   + square pos.(x):      " << setup.squareXPosition * setup.dxC << " [m]" <<
                                "\n   + square pos.(y):      " << setup.squareYPosition * setup.dxC << " [m]" <<
                                "\n   + square pos.(z):      " << setup.squareZPosition * setup.dxC << " [m]" <<
                                "\n   + square half-side:       " << setup.squareHalfSide * setup.dxC << " [m]" <<
   ...
   square.setupBoundary(blocks, ids.flagField);
   ...
   std::function< VelocityField_T::value_type (const Cell&, const Cell&, const shared_ptr< StructuredBlockForest >&, IBlock&) >
      wallDistanceFunctor = wallDistanceSquare(square);
   ...
   LBMMeshRefinement.addPostBoundaryHandlingFunction(evaluation->forceCalculationFunctor(), "Force Evaluation");
   ...
   LBMRefinement->addPostBoundaryHandlingFunction(evaluation->forceCalculationFunctor(), "Force Evaluation");
   ...
   {
      const std::string vtkFolder = "VTKSquareRE_" + std::to_string(uint64_c(setup.reynoldsNumber));
      auto vtkOutput =
   ...
   timeloop.addFuncBeforeTimeStep( evaluation->resetForceFunctor(), "Reset Force" );
